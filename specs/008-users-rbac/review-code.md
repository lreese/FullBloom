...
### 1. No database migration for the `users` table
...
### 2. Invite endpoint does not call Supabase's invite API
...
### 3. Salespeople fetch double-unwraps the API response (Bug)
...
### 4. Read permission checks missing on most GET endpoints
...
### 5. No `data_manager` test fixture or role-specific tests
...
### 6. Frontend `ProtectedRoute` only checks authentication, not authorization
...
### 7. Role validation could use `Literal` type instead of `model_post_init`
...
### 8. `ProfilePage` state doesn't sync with auth context updates
...
### 9. No error handling on role change/deactivate in UsersPage
...
### 10. `UserWithPermissionsResponse` missing `created_at`
...
| User model | Done | **Missing aerich migration** |
| Auth dependencies | Done | Idiomatic FastAPI pattern |
| Auth/users/profile routers | Done | **Invite doesn't call Supabase** |
...
| Users admin page | Done | Missing error handling on some actions |
| Profile page | Done | |
| Salesperson dropdown | **Bug** | Double-unwrapped API response |
| Database migration | **Missing** | |
| Supabase invite integration | **Missing** | Only creates local record |
