...
async def test_list_price_lists(async_client):
    ...
    resp = await async_client.get("/api/v1/price-lists")
...
async def test_list_price_lists_filter_inactive(async_client):
    ...
    resp = await async_client.get("/api/v1/price-lists?active=false")
    assert resp.status_code == 200
    data = resp.json()["data"]
    assert len(data) == 1
...
async def test_create_price_list_from_retail(async_client, sales_item):
    """POST /price-lists creates a list and copies from retail prices."""
    resp = await async_client.post(
        "/api/v1/price-lists", json={"name": "New List"}
    )
...
async def test_create_price_list_copy_from(async_client, price_list_with_items, sales_item):
    """POST /price-lists with copy_from copies from another list."""
    resp = await async_client.post(
        "/api/v1/price-lists",
        json={"name": "Copied List", "copy_from": str(price_list_with_items.id)},
    )
    assert resp.status_code == 201
...
async def test_create_price_list_duplicate_name(async_client):
    """POST /price-lists with duplicate name returns 422."""
    await PriceList.create(name="Dupe")
    resp = await async_client.post("/api/v1/price-lists", json={"name": "Dupe"})
...
async def test_rename_price_list(async_client, price_list):
    """PATCH /price-lists/{id} renames the list."""
    resp = await async_client.patch(
        f"/api/v1/price-lists/{price_list.id}", json={"name": "Renamed"}
    )
    assert resp.status_code == 200
    assert resp.json()["data"]["name"] == "Renamed"


async def test_rename_price_list_not_found(async_client):
    """PATCH /price-lists/{id} with fake ID returns 404."""
    resp = await async_client.patch(
        f"/api/v1/price-lists/{uuid.uuid4()}", json={"name": "Nope"}
    )
...
async def test_archive_price_list_converts_overrides(async_client, variety):
    ...
    resp = await async_client.post(f"/api/v1/price-lists/{pl.id}/archive")
    assert resp.status_code == 200
...
async def test_matrix_returns_correct_structure(async_client, price_list_with_items, sales_item):
    """GET /price-lists/matrix returns matrix with correct shape."""
    resp = await async_client.get("/api/v1/price-lists/matrix")
    assert resp.status_code == 200
...
async def test_update_price_list_item(async_client, price_list_with_items, sales_item):
...
async def test_update_price_list_item_creates_if_missing(async_client, price_list, sales_item):
...
async def test_bulk_update_price_list_items(async_client, price_list_with_items, sales_item):
...
async def test_update_retail_price(async_client, sales_item):
    """PATCH /price-lists/matrix/retail updates SalesItem.retail_price."""
    resp = await async_client.patch(
        "/api/v1/price-lists/matrix/retail",
        json={"sales_item_id": str(sales_item.id), "price": "15.00"},
    )
    assert resp.status_code == 200
...
async def test_impact_preview(async_client, price_list_with_items, sales_item, variety):
...
async def test_impact_preview_with_override(async_client, price_list_with_items, sales_item):
...
async def test_export_matrix_csv(async_client, price_list_with_items, sales_item):
    """GET /price-lists/matrix/export returns CSV."""
    resp = await async_client.get("/api/v1/price-lists/matrix/export")
    assert resp.status_code == 200
