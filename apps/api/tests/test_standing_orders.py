...
async def _create_standing_order_via_api(client, customer, sales_items, **overrides):
    ...
    resp = await client.post("/api/v1/standing-orders", json=payload)
...
@pytest.mark.anyio
async def test_create_standing_order(async_client, customer, sales_items):
    data = await _create_standing_order_via_api(async_client, customer, sales_items)
...
@pytest.mark.anyio
async def test_create_invalid_customer(async_client, sales_items):
    si_a, _ = sales_items
    payload = {
    ...
    resp = await async_client.post("/api/v1/standing-orders", json=payload)
    assert resp.status_code == 422
    assert "not found" in resp.json()["error"].lower()


@pytest.mark.anyio
async def test_create_empty_lines(async_client, customer):
    payload = {
        "customer_id": str(customer.id),
        "frequency_weeks": 1,
        "days_of_week": [0],
...
@pytest.mark.anyio
async def test_create_invalid_frequency(async_client, customer, sales_items):
    si_a, _ = sales_items
    payload = {
        "customer_id": str(customer.id),
        "frequency_weeks": 3,
...
@pytest.mark.anyio
async def test_create_invalid_days_of_week(async_client, customer, sales_items):
    si_a, _ = sales_items
    payload = {
        "customer_id": str(customer.id),
        "frequency_weeks": 1,
...
@pytest.mark.anyio
async def test_list_default_active_only(async_client, customer, sales_items):
    await _create_standing_order_via_api(async_client, customer, sales_items)
    # Create a second and pause it
    data2 = await _create_standing_order_via_api(
        async_client, customer, sales_items, days_of_week=[1]
    )
    ...
    resp = await async_client.get("/api/v1/standing-orders")
...
@pytest.mark.anyio
async def test_list_filter_all(async_client, customer, sales_items):
    await _create_standing_order_via_api(async_client, customer, sales_items)
    data2 = await _create_standing_order_via_api(
        async_client, customer, sales_items, days_of_week=[1]
    )
    ...
    resp = await async_client.get("/api/v1/standing-orders?status=all")
    assert resp.status_code == 200
    items = resp.json()["data"]
    assert len(items) == 2
...
@pytest.mark.anyio
async def test_list_filter_paused(async_client, customer, sales_items):
    data = await _create_standing_order_via_api(async_client, customer, sales_items)
    await async_client.post(
    ...
    resp = await async_client.get("/api/v1/standing-orders?status=paused")
    assert resp.status_code == 200
    items = resp.json()["data"]
    assert len(items) == 1
...
@pytest.mark.anyio
async def test_list_search_by_customer_name(async_client, customer, customer_b, sales_items):
    await _create_standing_order_via_api(async_client, customer, sales_items)
    await _create_standing_order_via_api(async_client, customer_b, sales_items)

    resp = await async_client.get("/api/v1/standing-orders?status=all&search=Boutique")
    assert resp.status_code == 200
    items = resp.json()["data"]
    assert len(items) == 1
...
@pytest.mark.anyio
async def test_list_filter_by_salesperson(async_client, customer, sales_items):
    await _create_standing_order_via_api(
        async_client, customer, sales_items, salesperson_email="alice@test.com"
    )
    await _create_standing_order_via_api(
    ...
    resp = await async_client.get(
        "/api/v1/standing-orders?status=all&salesperson_email=alice"
    )
    assert resp.status_code == 200
    items = resp.json()["data"]
    assert len(items) == 1
...
@pytest.mark.anyio
async def test_get_detail(async_client, customer, sales_items):
    created = await _create_standing_order_via_api(async_client, customer, sales_items)
    resp = await async_client.get(f"/api/v1/standing-orders/{created['id']}")
    assert resp.status_code == 200
    data = resp.json()["data"]
...
@pytest.mark.anyio
async def test_get_detail_404(async_client):
    resp = await async_client.get(f"/api/v1/standing-orders/{uuid.uuid4()}")
...
@pytest.mark.anyio
async def test_update_header_fields(async_client, customer, sales_items):
    created = await _create_standing_order_via_api(async_client, customer, sales_items)
...
@pytest.mark.anyio
async def test_update_lines(async_client, customer, sales_items):
    si_a, si_b = sales_items
    created = await _create_standing_order_via_api(async_client, customer, sales_items)

    # Get existing lines
    detail = await async_client.get(f"/api/v1/standing-orders/{created['id']}")
...
@pytest.mark.anyio
async def test_update_creates_audit_log(async_client, customer, sales_items):
    created = await _create_standing_order_via_api(async_client, customer, sales_items)
    await async_client.put(
        f"/api/v1/standing-orders/{created['id']}",
        json={"ship_via": "UPS", "reason": "Changed carrier"},
    )

    resp = await async_client.get(
        f"/api/v1/standing-orders/{created['id']}/audit-log"
    )
    assert resp.status_code == 200
...
@pytest.mark.anyio
async def test_update_409_when_paused(async_client, customer, sales_items):
    created = await _create_standing_order_via_api(async_client, customer, sales_items)
    await async_client.post(
    ...
    resp = await async_client.put(
        f"/api/v1/standing-orders/{created['id']}",
        json={"ship_via": "UPS", "reason": "try update"},
    )
    assert resp.status_code == 409


@pytest.mark.anyio
async def test_update_409_when_cancelled(async_client, customer, sales_items):
    created = await _create_standing_order_via_api(async_client, customer, sales_items)
    await async_client.post(
...
@pytest.mark.anyio
async def test_pause_happy_path(async_client, customer, sales_items):
    created = await _create_standing_order_via_api(async_client, customer, sales_items)
    resp = await async_client.post(
        f"/api/v1/standing-orders/{created['id']}/pause",
        json={"reason": "Customer on vacation"},
    )
    assert resp.status_code == 200
...
@pytest.mark.anyio
async def test_pause_409_when_not_active(async_client, customer, sales_items):
    created = await _create_standing_order_via_api(async_client, customer, sales_items)
    # Pause first
    await async_client.post(
        f"/api/v1/standing-orders/{created['id']}/pause",
...
@pytest.mark.anyio
async def test_pause_audit_log(async_client, customer, sales_items):
    created = await _create_standing_order_via_api(async_client, customer, sales_items)
    await async_client.post(
        f"/api/v1/standing-orders/{created['id']}/pause",
        json={"reason": "vacation"},
...
@pytest.mark.anyio
async def test_resume_happy_path(async_client, customer, sales_items):
    created = await _create_standing_order_via_api(async_client, customer, sales_items)
    await async_client.post(
        f"/api/v1/standing-orders/{created['id']}/pause",
        json={"reason": "temp"},
...
@pytest.mark.anyio
async def test_resume_409_when_active(async_client, customer, sales_items):
    created = await _create_standing_order_via_api(async_client, customer, sales_items)
    resp = await async_client.post(
        f"/api/v1/standing-orders/{created['id']}/resume",
        json={},
...
@pytest.mark.anyio
async def test_resume_409_when_cancelled(async_client, customer, sales_items):
    created = await _create_standing_order_via_api(async_client, customer, sales_items)
    await async_client.post(
        f"/api/v1/standing-orders/{created['id']}/cancel",
        json={"reason": "closed"},
...
@pytest.mark.anyio
async def test_resume_audit_log(async_client, customer, sales_items):
    created = await _create_standing_order_via_api(async_client, customer, sales_items)
    await async_client.post(
        f"/api/v1/standing-orders/{created['id']}/pause",
        json={"reason": "temp"},
...
@pytest.mark.anyio
async def test_cancel_happy_path(async_client, customer, sales_items):
    created = await _create_standing_order_via_api(async_client, customer, sales_items)
    resp = await async_client.post(
        f"/api/v1/standing-orders/{created['id']}/cancel",
        json={"reason": "Customer account closed"},
    )
    assert resp.status_code == 200
...
@pytest.mark.anyio
async def test_cancel_409_when_already_cancelled(async_client, customer, sales_items):
    created = await _create_standing_order_via_api(async_client, customer, sales_items)
    await async_client.post(
        f"/api/v1/standing-orders/{created['id']}/cancel",
        json={"reason": "first"},
...
@pytest.mark.anyio
async def test_cancel_from_paused(async_client, customer, sales_items):
    created = await _create_standing_order_via_api(async_client, customer, sales_items)
    await async_client.post(
        f"/api/v1/standing-orders/{created['id']}/pause",
        json={"reason": "temp"},
...
@pytest.mark.anyio
async def test_cancel_audit_log(async_client, customer, sales_items):
    created = await _create_standing_order_via_api(async_client, customer, sales_items)
    await async_client.post(
        f"/api/v1/standing-orders/{created['id']}/cancel",
        json={"reason": "closing"},
...
@pytest.mark.anyio
async def test_generate_preview_cadence_match(async_client, customer, sales_items):
    """A weekly Mon/Wed/Fri order should match Mon Apr 13, Wed Apr 15, Fri Apr 17."""
    await _create_standing_order_via_api(async_client, customer, sales_items)

    resp = await async_client.post(
        "/api/v1/standing-orders/generate-preview",
        json={"date_from": "2026-04-13", "date_to": "2026-04-17"},
    )
    assert resp.status_code == 200
    data = resp.json()["data"]
...
@pytest.mark.anyio
async def test_generate_preview_two_week_cycle(async_client, customer, sales_items):
    ...
    resp = await async_client.post(
        "/api/v1/standing-orders/generate-preview",
        json={"date_from": "2026-04-13", "date_to": "2026-04-27"},
    )
    assert resp.status_code == 200
...
@pytest.mark.anyio
async def test_generate_preview_duplicate_detection(async_client, customer, sales_items):
...
@pytest.mark.anyio
async def test_generate_orders_creates_linked_orders(async_client, customer, sales_items):
    created = await _create_standing_order_via_api(async_client, customer, sales_items)
...
@pytest.mark.anyio
async def test_generate_orders_skips_duplicates(async_client, customer, sales_items):
    await _create_standing_order_via_api(async_client, customer, sales_items)
...
@pytest.mark.anyio
async def test_generate_orders_excludes_paused(async_client, customer, sales_items):
    created = await _create_standing_order_via_api(async_client, customer, sales_items)
    await async_client.post(
        f"/api/v1/standing-orders/{created['id']}/pause",
        json={"reason": "vacation"},
...
@pytest.mark.anyio
async def test_generate_orders_multi_day(async_client, customer, sales_items):
...
@pytest.mark.anyio
async def test_audit_log_list(async_client, customer, sales_items):
    created = await _create_standing_order_via_api(async_client, customer, sales_items)
    resp = await async_client.get(
        f"/api/v1/standing-orders/{created['id']}/audit-log"
    )
...
@pytest.mark.anyio
async def test_audit_log_404(async_client):
    resp = await async_client.get(
        f"/api/v1/standing-orders/{uuid.uuid4()}/audit-log"
    )
    assert resp.status_code == 404
...
@pytest.mark.anyio
async def test_update_404(async_client):
    resp = await async_client.put(
        f"/api/v1/standing-orders/{uuid.uuid4()}",
        json={"ship_via": "UPS", "reason": "test"},
    )
    assert resp.status_code == 404


@pytest.mark.anyio
async def test_pause_404(async_client):
    resp = await async_client.post(
...
@pytest.mark.anyio
async def test_resume_404(async_client):
    resp = await async_client.post(
...
@pytest.mark.anyio
async def test_cancel_404(async_client):
    resp = await async_client.post(
...
@pytest.mark.anyio
async def test_update_zero_stems_422(async_client, customer, sales_items):
    si_a, _ = sales_items
    created = await _create_standing_order_via_api(async_client, customer, sales_items)
    detail = await async_client.get(f"/api/v1/standing-orders/{created['id']}")
    lines = detail.json()["data"]["lines"]
...
@pytest.mark.anyio
async def test_generate_preview_four_week_cycle(async_client, customer, sales_items):
    ...
    resp = await async_client.post(
        "/api/v1/standing-orders/generate-preview",
        json={"date_from": "2026-04-13", "date_to": "2026-05-11"},
    )
    assert resp.status_code == 200
    dates = [m["generate_date"] for m in resp.json()["data"]["matches"]]
...
@pytest.mark.anyio
async def test_generate_no_matching_dates(async_client, customer, sales_items):
...
@pytest.mark.anyio
async def test_apply_to_future_orders(async_client, customer, sales_items):
    ...
    gen_resp = await async_client.post(
        "/api/v1/standing-orders/generate",
        json={"date_from": "2026-04-20", "date_to": "2026-04-20"},
    )
...
@pytest.mark.anyio
async def test_generate_skip_already_generated_false(async_client, customer, sales_items):
...
@pytest.mark.anyio
async def test_update_add_new_line(async_client, customer, sales_items):
