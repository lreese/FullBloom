...
async def test_search_by_name_partial(async_client):
    ...
    resp = await async_client.get("/api/v1/customers", params={"search": "sunrise"})
...
async def test_search_by_customer_number(async_client):
    ...
    resp = await async_client.get("/api/v1/customers", params={"search": "456"})
    assert resp.status_code == 200
    data = resp.json()["data"]
    assert len(data) == 1
...
async def test_search_case_insensitive(async_client):
    ...
    resp = await async_client.get("/api/v1/customers", params={"search": "OREGON"})
    assert resp.status_code == 200
    data = resp.json()["data"]
    assert len(data) == 1
...
async def test_search_no_results(async_client):
    """Search with no matching term returns empty list."""
    await _create_customer(300, "Valley Growers")

    resp = await async_client.get("/api/v1/customers", params={"search": "nonexistent"})
    assert resp.status_code == 200
    data = resp.json()["data"]
    assert len(data) == 0


async def test_no_search_returns_all(async_client):
...
async def test_search_with_active_false(async_client):
    ...
    resp = await async_client.get(
        "/api/v1/customers", params={"active": "false", "search": "sunset"}
    )
    assert resp.status_code == 200
    data = resp.json()["data"]
    assert len(data) == 1
...
async def test_search_with_active_omitted(async_client):
    ...
    resp = await async_client.get("/api/v1/customers", params={"search": "river"})
    assert resp.status_code == 200
    data = resp.json()["data"]
    assert len(data) == 1
