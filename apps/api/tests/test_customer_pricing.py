...
async def test_customer_pricing_grid(async_client, setup_pricing, auth_headers_admin):
    ...
    resp = await async_client.get(f"/api/v1/customers/{customer.id}/pricing", headers=auth_headers_admin)
...
async def test_customer_pricing_with_override(async_client, setup_pricing, auth_headers_admin):
...
async def test_customer_pricing_retail_fallback(async_client, sales_item, auth_headers_admin):
...
async def test_customer_pricing_not_found(async_client, auth_headers_admin):
    """404 when customer does not exist."""
    resp = await async_client.get(f"/api/v1/customers/{uuid.uuid4()}/pricing", headers=auth_headers_admin)
...
async def test_set_customer_price_create(async_client, customer_with_list, sales_item, auth_headers_admin):
    """POST /customers/{id}/prices creates a new override."""
    resp = await async_client.post(
        f"/api/v1/customers/{customer_with_list.id}/prices",
        json={"sales_item_id": str(sales_item.id), "price": "7.50"}, headers=auth_headers_admin)
...
async def test_set_customer_price_update(async_client, setup_pricing, auth_headers_admin):
    ...
    resp = await async_client.post(
        ...
        json={"sales_item_id": str(si.id), "price": "9.00"}, headers=auth_headers_admin)
    assert resp.status_code == 200
    assert float(resp.json()["data"]["price"]) == 9.00


async def test_remove_customer_price(async_client, setup_pricing, auth_headers_admin):
    ...
    resp = await async_client.delete(
        f"/api/v1/customers/{customer.id}/prices/{si.id}", headers=auth_headers_admin)
...
async def test_remove_customer_price_not_found(async_client, customer_with_list, sales_item, auth_headers_admin):
    """DELETE returns 404 when no override exists."""
    resp = await async_client.delete(
        f"/api/v1/customers/{customer_with_list.id}/prices/{sales_item.id}", headers=auth_headers_admin)
    assert resp.status_code == 404
...
async def test_bulk_set_price(async_client, setup_pricing, auth_headers_admin):
...
async def test_bulk_remove_overrides(async_client, setup_pricing, auth_headers_admin):
...
async def test_bulk_reset_to_list(async_client, setup_pricing, auth_headers_admin):
...
async def test_item_pricing_view(async_client, setup_pricing, auth_headers_admin):
    """GET /sales-items/{id}/customer-pricing returns all customers' prices."""
    si = setup_pricing["sales_item"]
    customer = setup_pricing["customer"]

    resp = await async_client.get(f"/api/v1/sales-items/{si.id}/customer-pricing", headers=auth_headers_admin)
    assert resp.status_code == 200
...
async def test_item_pricing_not_found(async_client, auth_headers_admin):
    """404 when sales item does not exist."""
    resp = await async_client.get(f"/api/v1/sales-items/{uuid.uuid4()}/customer-pricing", headers=auth_headers_admin)
    assert resp.status_code == 404
...
async def test_anomaly_flag(async_client, setup_pricing, auth_headers_admin):
...
async def test_no_anomaly_within_threshold(async_client, setup_pricing, auth_headers_admin):
...
async def test_export_customer_pricing_csv(async_client, setup_pricing, auth_headers_admin):
    """GET /customers/{id}/pricing/export returns CSV."""
    customer = setup_pricing["customer"]
    resp = await async_client.get(f"/api/v1/customers/{customer.id}/pricing/export", headers=auth_headers_admin)
    assert resp.status_code == 200
