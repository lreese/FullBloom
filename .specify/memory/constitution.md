...
Migrations MUST always be generated by Aerich (`aerich migrate`), never hand-written.

Every new `APIRouter()` MUST include `dependencies=[Depends(get_current_user)]` at the router
level so all endpoints require authentication by default. Permission checks (read vs write) are
added per-endpoint as needed using `require_permission(area, action)`. No router may be created
without router-level auth — this is enforced by convention and code review. Aerich
tracks model state via `MODELS_STATE` in each migration file — hand-written migrations lack this
and break Aerich's ability to generate future migrations. If a complex data migration is needed
(backfill, rename + add column), generate the structural migration via Aerich first, then add
data manipulation SQL to the generated file's upgrade function.
...
### Role Access (NON-NEGOTIABLE)

Every spec that introduces a new sidebar-level feature MUST include a row in the RBAC permission
matrix defining access levels (Read/Write, View only, No access) for all four roles: admin,
salesperson, data_manager, field_worker. This ensures role-based access is designed upfront, not
